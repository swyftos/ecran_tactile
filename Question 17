...
// Calibration data for raw touch to screen coordinates
#define TS_MINX 150
#define TS_MINY 130
#define TS_MAXX 3800
#define TS_MAXY 4000
...
const int correctCode = 1234; // Code correct du digicode

int enteredCode = 0; // Code saisi
int currentMultiplier = 1000; // Multiplicateur initial (4 chiffres)
int wrongAttempts = 0; // Nombre de tentatives incorrectes
unsigned long blockStartTime = 0; // Temps de début du blocage
const int blockDuration = 10000; // Durée du blocage (en ms)

int buttonWidth = 80; // Largeur d'un bouton
int buttonHeight = 80; // Hauteur d'un bouton
bool isBlocked = false; // Statut du digicode
...
void blockDigicode() {
  ...
  tft.setTextColor(ILI9341_BLACK);
  tft.setTextSize(2);
  tft.print("Digicode bloque!");
}

void unblockDigicode() {
  isBlocked = false;
  wrongAttempts = 0; // Réinitialiser les tentatives
  tft.fillScreen(ILI9341_BLACK);
  drawKeyboard();
}

void loop() {
  // Vérifier si le digicode est bloqué
  if (isBlocked) {
      ...
      if (!isTouching || row != lastRow || col != lastCol) {
        ...
        if (currentMultiplier == 0) {
          if (enteredCode == correctCode) {
            ...
            wrongAttempts++;
            if (wrongAttempts >= 3) {
              blockDigicode(); // Bloquer après 3 tentatives erronées
            }
          }
